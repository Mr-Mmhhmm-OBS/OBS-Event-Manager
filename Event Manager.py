...
def event_callback_stop_event(event):
	if event == obs.OBS_FRONTEND_EVENT_SCENE_CHANGED:
		if current_scene_name() == closing_scene:
			if auto_recording and obs.obs_frontend_recording_active():
				if recording_delay == 0:
					stop_recording()
				else:
					obs.timer_add(stop_recording, recording_delay * 1000)
			if auto_streaming and obs.obs_frontend_streaming_active():
...
def stop_streaming():
	if auto_streaming and obs.obs_frontend_streaming_active() and current_scene_name() == closing_scene:
		obs.obs_frontend_streaming_stop()
	obs.timer_remove(stop_streaming)

def stop_recording():
	if auto_recording and obs.obs_frontend_recording_active() and current_scene_name() == closing_scene:
...
def closing_scene_modified(props, properties, settings):
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "auto_stop_streaming"), closing_scene != "")
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "stop_streaming_delay"), closing_scene != "" and auto_stop_streaming)
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "auto_stop_recording"), closing_scene != "")
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "stop_recording_delay"), closing_scene != "" and auto_stop_recording)
	return True

def auto_stop_streaming_modified(props, properties, settings):
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "stop_streaming_delay"), closing_scene != "" and auto_stop_streaming)
	return True

def auto_stop_recording_modified(props, properties, settings):
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "stop_recording_delay"), closing_scene != "" and auto_stop_recording)
	return True

def on_property_modified(props, property, settings):
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "start_scene"), auto_start_streaming or auto_start_recording)
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "weekday"), auto_start_streaming or auto_start_recording)
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "livestream_start"), auto_start_streaming or auto_start_recording)
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "preshow_duration"), auto_start_streaming or auto_start_recording)
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "text_source"), auto_start_streaming or auto_start_recording)
	obs.obs_property_set_enabled(obs.obs_properties_get(props, "countdown_final_text"), (auto_start_streaming or auto_start_recording) and text_source != "")
	return True
...
def script_properties():
	...
	p = obs.obs_properties_add_bool(auto_start_group, "auto_start_streaming", "Auto Start Streaming")
	obs.obs_property_set_modified_callback(p, on_property_modified)
	p = obs.obs_properties_add_bool(auto_start_group, "auto_start_recording", "Auto Start Recording")
	obs.obs_property_set_modified_callback(p, on_property_modified)
	...
	obs.obs_property_set_enabled(p, auto_start_streaming or auto_start_recording)
	...
	obs.obs_property_set_enabled(text_source_list, auto_start_streaming or auto_start_recording)
	...
	obs.obs_property_set_enabled(p, (auto_start_streaming or auto_start_recording) and text_source != "")
	...
	p = obs.obs_properties_add_bool(auto_stop_group, "auto_stop_streaming", "Auto Stop Streaming")
	obs.obs_property_set_modified_callback(p, auto_stop_streaming_modified)
	obs.obs_properties_add_int(auto_stop_group, "stop_streaming_delay", "Streaming Delay", 0, 300, 15)

	p = obs.obs_properties_add_bool(auto_stop_group, "auto_stop_recording", "Auto Stop Recording")
	obs.obs_property_set_modified_callback(p, auto_stop_recording_modified)
	obs.obs_properties_add_int(auto_stop_group, "stop_recording_delay", "Recording Delay", 0, 120, 5)
...
def script_defaults(settings):
	...
	obs.obs_data_set_default_bool(settings, "auto_stop_streaming", auto_stop_streaming)
	obs.obs_data_set_default_int(settings, "stop_streaming_delay", stop_streaming_delay)
	obs.obs_data_set_default_bool(settings, "auto_stop_recording", auto_stop_recording)
	obs.obs_data_set_default_int(settings, "stop_recording_delay", stop_recording_delay)
...
def script_update(settings):
	...
	global auto_stop_streaming
	auto_stop_streaming = obs.obs_data_get_bool(settings, "auto_stop_streaming")
	global stop_streaming_delay
	stop_streaming_delay = obs.obs_data_get_int(settings, "stop_streaming_delay")
	global auto_stop_recording
	auto_stop_recording = obs.obs_data_get_bool(settings, "auto_stop_recording")
...
def script_load(settings):
	...
	if (auto_start_streaming or auto_start_recording) and weekday == datetime.now().weekday() and diff_time().total_seconds() > 0 and not (obs.obs_frontend_streaming_active() or obs.obs_frontend_recording_active()):
